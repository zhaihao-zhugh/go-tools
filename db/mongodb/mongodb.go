...
func NewClient(host string, port int, username, password, database string) {
	cli, err := mongo.Connect(context.Background(), options.Client().ApplyURI(fmt.Sprintf("mongodb://%s:%s@%s:%d", username, password, host, port)))
	if err != nil {
		panic(err)
	}
